...
![img](apollo_构建minbox.assets/object_building.png)
...
![img](apollo_构建minbox.assets/minbox_framework.png)
...
virtual void SetDefaultValue(pcl_util::PointCloudPtr cloud,
                               ...
                               Eigen::Vector4f* min_pt,
                               Eigen::Vector4f* max_pt) {
    // 这个函数 GetCloudMinMax3D() 就是把点云cloud中xyz三轴 的极值 存在 min_pt 和 max_pt中
    // min_pt[x_min,y_min,z_min]
    // max_pt[x_max,y_max,z_max]
...
> 完成初始化的操作,或得了cloud的min_pt和max_pt,存在min_pt和max_pt中
>
> 对象obj中的 length,width,height和center由min_pt和max_pt也可以获得
>
> 同时,如果点数小于4,初始化cloud的polygon

点数小于4,函数`ComputePolygon2dxy`返回,下面看点数大于4的情况:
...
```C++
  ...
  /// apollo 3.5 中,对下面这段代码进行了封装 判断点云 是否
...
![img](apollo_构建minbox.assets/minbox_polygons.png)

这个过程只要自己稍加关注一点就可以了解他的原理和过程，这里不再过度解释每个模块。上图是计算多边形交点的流程示意图。

## Step 2. MinBox构建--边框构建

![img](apollo_构建minbox.assets/minbox_box.png)
...
![img](apollo_构建minbox.assets/minbox_maxminpt.png)
...
![img](apollo_构建minbox.assets/minbox_edge_selection.png)
